            ...
            <div class="space-y-8">
                ...
                <div class="lg:col-span-2 space-y-6">
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                                ...
                                <div x-show="open" @click.away="open = false" x-transition
                                    ...
                                    <button type="button" wire:click="addProduct({{ $product->id }})"
                                        ...
                                        <svg class="w-5 h-5 text-indigo-500" fill="currentColor" viewBox="0 0 20 20">
                                            <path
                                                d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-11H9v2H7v2h2v2h2v-2h2V9h-2V7z" />
                                        </svg>
                                    </button>
                                    @empty
                                    <div class="px-4 py-6 text-center text-gray-500 text-sm italic">Produk tidak
                                        ditemukan...</div>
                                    @endforelse
                                </div>
                            </div>
                        ...
                        @if(!$outletId)
                        <p class="mt-2 text-xs text-amber-600 font-medium">* Pilih outlet terlebih dahulu untuk memuat
                            produk</p>
                        @endif
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    <tr class="hover:bg-gray-50 transition-colors">
                                        <td class="px-6 py-4">
                                            <div class="text-sm font-bold text-gray-900">{{ $item['product_name'] }}
                                            </div>
                                        ...
                                        <td class="px-6 py-4 text-center font-bold text-gray-600">
                                            {{ number_format($item['system_quantity']) }} <span
                                                class="text-[10px] font-normal uppercase">{{ $item['unit'] }}</span>
                                        </td>
                                        <td class="px-6 py-4">
                                            <input type="number"
                                                wire:model.live.debounce.1000ms="selectedProducts.{{ $index }}.physical_quantity"
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            ...
                                            <span
                                                class="px-2.5 py-1 rounded-full text-xs font-black {{ $diff == 0 ? 'bg-gray-100 text-gray-500' : ($diff > 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700') }}">
                                                {{ $diff > 0 ? '+' : '' }}{{ $diff }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4">
                                            ...
                                            @error("selectedProducts.{$index}.reason") <span
                                                class="text-[10px] text-red-500 block mt-1">{{ $message }}</span>
                                            @enderror
                                            @else
                                            <span class="text-[10px] text-gray-400 font-medium italic">Data
                                                Sesuai</span>
                                            @endif
                                        </td>
                        ...
                        <div class="p-12 text-center">
                            <div
                                class="bg-indigo-50 rounded-full w-20 h-20 flex items-center justify-center mx-auto mb-4">
                            ...
                            <p class="text-sm text-gray-500 max-w-xs mx-auto">Silakan cari produk atau muat semua produk
                                untuk memulai proses audit fisik.</p>
                        </div>
                        @endif
                    </div>
                    @if(count($selectedProducts) > 0)
                    <div class="bg-indigo-900 rounded-2xl shadow-lg p-6 text-white">
                        <h3 class="text-md font-bold mb-4 opacity-80 uppercase tracking-wider text-xs">Ringkasan Audit
                        </h3>
                        <div class="space-y-4">
                            ...
                            <div class="flex justify-between items-center text-green-300">
                                ...
                                <span
                                    class="font-bold">{{ collect($selectedProducts)->filter(fn($i) => $i['difference'] > 0)->count() }}</span>
                            </div>
                    ...
                    <div
                        class="flex flex-col-reverse sm:flex-row items-center justify-end space-y-3 space-y-reverse sm:space-y-0 sm:space-x-4 pt-8 border-t border-gray-100">
