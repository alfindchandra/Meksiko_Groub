                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <p class="text-sm text-yellow-700">
                            <strong>Perhatian:</strong> Terdapat audit belum selesai ({{ $pendingAudit->status == 'payment_submitted' ? 'Menunggu Konfirmasi' : 'Menunggu Pembayaran' }}) pada tanggal {{ $pendingAudit->audited_at->format('d M Y') }}. 
                            Selisih stok: 
                            @if($pendingAudit->difference > 0)
                                <span class="font-bold text-green-600">+{{ $pendingAudit->difference }}</span>
                            @else
                                <span class="font-bold text-red-600">{{ $pendingAudit->difference }}</span>
                            @endif
                        </p>
                    </div>
                </div>
