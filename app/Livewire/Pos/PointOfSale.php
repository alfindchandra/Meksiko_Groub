...
class PointOfSale extends Component
{
    ...
    public function addToCart($productId, $variantId = null)
    {
        ...
        // Jika produk punya varian dan belum dipilih → tampilkan modal pilih varian
        ...
        if (!$stock || $stock->quantity < 1) {
            $this->dispatch('notify', ['type' => 'error', 'message' => 'Stok tidak tersedia']);
            return;
        }

        // Tentukan harga & satuan dari varian atau produk dasar
    ...
    private function recalculateCartItem($index)
    {
        ...
        $discountAmount  = $subtotalBefore * ($discountPercentage / 100);
        ...
        $this->cart[$index]['subtotal']                 = $subtotalAfter;
    }
    ...
    public function getTotalItemDiscountProperty(): float
    {
    ...
    public function checkout()
    {
        ...
        try {
            ...
            $sale = Sale::create([
                ...
                'subtotal'       => (float) $this->subtotal + (float) $this->totalItemDiscount,
            ...
            foreach ($this->cart as $item) {
                SaleItem::create([
                    ...
                    'unit_name'  => $item['product_unit'] ?? null,
                    'quantity'   => (int) $item['quantity'],
                ...
                if ($stock) {
                    ...
                    StockHistory::create([
                        ...
                        'user_id'         => auth()->id(),
                        'notes'           => "Penjualan {$saleNumber}" . ($item['variant_id'] ? " [{$item['product_unit']}]" : ''),
                    ]);
                }
            }
    ...
    public function render()
    {
        ...
        if (strlen($this->searchProduct) >= 2) {
            $products = Product::with('activeVariants', 'activeDiscountTiers')
