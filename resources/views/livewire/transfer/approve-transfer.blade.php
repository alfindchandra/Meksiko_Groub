<div x-data="{ 
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6">
            ...
            <div class="p-6 sm:p-8">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500">Diajukan pada {{ $transfer->created_at->format('d M Y, H:i') }}</p>
                        </div>
                    </div>
                    <span class="inline-flex items-center px-4 py-1.5 rounded-full text-xs font-black bg-amber-50 text-amber-700 border border-amber-100 uppercase tracking-widest">
                        <span class="w-2 h-2 bg-amber-400 rounded-full mr-2 animate-pulse"></span>
                        Menunggu Persetujuan
                    </span>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-3 gap-8 p-6 bg-gray-50 rounded-2xl border border-gray-100 relative">
                    <div class="text-center md:text-left">
                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Outlet Pengirim (Asal)</p>
                        <p class="text-lg font-bold text-gray-900">{{ $transfer->fromOutlet->name }}</p>
                        <p class="text-sm text-gray-500 font-medium">{{ $transfer->fromOutlet->code }} • {{ $transfer->fromOutlet->city }}</p>
                    </div>
                    
                    <div class="flex flex-col items-center justify-center">
                        <div class="bg-white p-2 rounded-full shadow-sm border border-gray-100">
                            <svg class="w-6 h-6 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                        </div>
                    </div>
                    
                    <div class="text-center md:text-right">
                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Outlet Penerima (Tujuan)</p>
                        <p class="text-lg font-bold text-gray-900">{{ $transfer->toOutlet->name }}</p>
                        <p class="text-sm text-gray-500 font-medium">{{ $transfer->toOutlet->code }} • {{ $transfer->toOutlet->city }}</p>
                    </div>
                </div>
                ...
                <div class="mt-6 flex items-center p-4 bg-indigo-50/50 rounded-2xl border border-indigo-100/50">
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-sm font-bold text-indigo-900">{{ $transfer->requestedBy->name }} <span class="font-normal text-indigo-500 ml-1">— {{ $transfer->requestedBy->role->display_name }}</span></p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-gray-200 overflow-hidden mb-6">
            <div class="p-6 border-b border-gray-50 flex justify-between items-center">
                ...
                <span class="text-xs font-bold px-2 py-1 bg-gray-100 text-gray-500 rounded-lg">{{ $transfer->items->count() }} Item</span>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-100">
                    <thead class="bg-gray-50/50">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-center text-[10px] font-black text-gray-400 uppercase tracking-widest">Stok Asal</th>
                            <th class="px-6 py-4 text-center text-[10px] font-black text-gray-400 uppercase tracking-widest">Kelayakan</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-50">
                        @foreach($transfer->items as $item)
                        @php
                            $stock = \App\Models\Stock::where('product_id', $item->product_id)->where('outlet_id', $transfer->from_outlet_id)->first();
                        ...
                        <tr class="{{ $canFulfill ? 'hover:bg-gray-50/50' : 'bg-rose-50/30' }} transition-colors">
                            <td class="px-6 py-4">
                                ...
                                <div class="flex items-center mt-0.5 space-x-2">
                                    <span class="text-[10px] font-mono text-gray-400">SKU: {{ $item->product->sku }}</span>
                                    <span class="text-gray-300">|</span>
                                    <span class="text-[10px] text-indigo-500 font-bold uppercase">{{ $item->product->category->name }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-center font-black text-gray-900">
                                {{ number_format($item->quantity_requested) }}
                                <span class="text-[10px] font-normal text-gray-400 ml-1">{{ $item->product->unit }}</span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <span class="text-sm font-black {{ $canFulfill ? 'text-emerald-600' : 'text-rose-600' }}">
        ...
        <div class="mb-6 p-4 bg-rose-600 rounded-2xl shadow-lg shadow-rose-100 flex items-center space-x-4 text-white">
            ...
            <div>
                ...
                <p class="text-xs opacity-90">Terdapat {{ $insufficientStock }} item yang permintaannya melebihi stok tersedia di outlet asal.</p>
            </div>
        </div>
        @endif

        <div class="bg-indigo-900 rounded-3xl p-6 sm:p-8 shadow-2xl shadow-indigo-200 flex flex-col md:flex-row items-center justify-between gap-6 overflow-hidden relative">
            ...
            <div class="absolute -right-10 -bottom-10 opacity-10 text-white">
                <svg class="w-48 h-48" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path></svg>
            </div>
        </div>
    </div>
    ...
    <div x-show="showApproveModal" x-cloak
        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white rounded-[2.5rem] shadow-2xl max-w-md w-full p-8 text-center border border-gray-100">
                ...
                <p class="text-sm text-gray-500 mb-8">Ini akan memberi sinyal kepada outlet pengirim untuk segera memproses fisik barang.</p>
                <div class="grid grid-cols-2 gap-3">
                    <button @click="showApproveModal = false" class="px-6 py-4 bg-gray-100 text-gray-500 font-bold rounded-2xl hover:bg-gray-200 transition-all">Batal</button>
                    <button wire:click="approve" class="px-6 py-4 bg-indigo-600 text-white font-bold rounded-2xl hover:bg-indigo-700 shadow-lg shadow-indigo-200 transition-all">Ya, Setujui</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div x-show="showRejectModal" x-cloak
        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white rounded-[2.5rem] shadow-2xl max-w-md w-full p-8 border border-gray-100">
                ...
                <p class="text-sm text-gray-500 mb-6">Mohon berikan alasan penolakan agar pemohon dapat memperbaiki permintaan di masa mendatang.</p>
                ...
                <div class="mb-8">
                    ...
                    @error('rejectionReason') <p class="mt-2 text-[10px] text-rose-500 font-black uppercase">{{ $message }}</p> @enderror
                </div>

                <div class="grid grid-cols-2 gap-3">
                    <button @click="showRejectModal = false" class="px-6 py-4 bg-gray-100 text-gray-500 font-bold rounded-2xl hover:bg-gray-200">Batal</button>
                    <button wire:click="reject" class="px-6 py-4 bg-rose-600 text-white font-bold rounded-2xl hover:bg-rose-700 shadow-lg shadow-rose-200 transition-all">Tolak Sekarang</button>
                </div>
            </div>
        </div>
    </div>
