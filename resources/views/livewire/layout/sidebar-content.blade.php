<div class="flex items-center h-16 px-6 overflow-hidden">
    <div class="flex items-center space-x-3 min-w-max">
        <div class="flex-shrink-0 flex items-center justify-center w-10 h-10 bg-primary-600 rounded-lg shadow-sm">
        ...
        <div x-show="{{ $isMobile ? 'true' : '!sidebarMinimized' }}" 
             ...
             class="flex flex-col">
            <h1 class="text-sm font-bold text-gray-900 leading-none">mkkkk</h1>
            <p class="text-[10px] text-gray-500 mt-1 uppercase tracking-wider">alfinnnnn</p>
        </div>
    </div>
</div>
    ...
    <div x-data="{ open: {{ request()->routeIs('stock.*') ? 'true' : 'false' }} }">
        <button @click="{{ !$isMobile ? 'sidebarMinimized ? sidebarMinimized = false : open = !open' : 'open = !open' }}" 
            ...
            <div class="flex items-center min-w-max">
                <svg class="w-5 h-5 flex-shrink-0 text-gray-400 group-hover:text-primary-600 {{ $isMobile ? 'mr-3' : '' }}"
                     :class="!{{ $isMobile ? 'true' : 'false' }} && sidebarMinimized ? 'mx-auto' : 'mr-3'"
                     fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>
                </svg>
        ...
        <div x-show="open && {{ $isMobile ? 'true' : '!sidebarMinimized' }}" 
             x-collapse class="ml-8 mt-1 space-y-1">
             ...
             x-collapse class="ml-8 mt-1 space-y-1">
                ...
                <a href="{{ route('transfer.create') }}" 
                   class="block px-3 py-2 text-sm rounded-lg transition-colors
                          {{ request()->routeIs('transfer.create') 
       ...
       class="block px-3 py-2 text-sm rounded-lg transition-colors
        ...
        Daftar Transaksi
                ...
                <a href="{{ route('transfer.pending') }}" 
                   class="block px-3 py-2 text-sm rounded-lg transition-colors
                          {{ request()->routeIs('transfer.pending') 
                              ? 'bg-primary-50 text-primary-700' 
                              : 'text-gray-600 hover:bg-gray-50' }}">
                    Pending Approval
       ...
       <a href="{{ route('shipment.list') }}" 
        ...
        <div x-data="{ open: {{ request()->routeIs('audit.*') ? 'true' : 'false' }} }">
            <button @click="{{ !$isMobile ? 'sidebarMinimized ? sidebarMinimized = false : open = !open' : 'open = !open' }}" 
                    class="flex items-center justify-between w-full px-3 py-2 text-sm font-medium text-gray-700 rounded-xl hover:bg-gray-100 transition-all group">
                <div class="flex items-center min-w-max">
                    <svg class="w-5 h-5 flex-shrink-0 transition-colors text-gray-400 group-hover:text-primary-600"
                         :class="!{{ $isMobile ? 'true' : 'false' }} && sidebarMinimized ? 'mx-auto' : 'mr-3'"
                         fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div x-show="open && {{ $isMobile ? 'true' : '!sidebarMinimized' }}" x-collapse class="ml-8 mt-1 space-y-1">
        ...
        @can('manage-users')
        <div x-data="{ open: {{ request()->routeIs('admin.data.*') || request()->routeIs('permissions.*') ? 'true' : 'false' }} }">
            <button @click="{{ !$isMobile ? 'sidebarMinimized ? sidebarMinimized = false : open = !open' : 'open = !open' }}" 
                    class="flex items-center justify-between w-full px-3 py-2 text-sm font-medium text-gray-700 rounded-xl hover:bg-gray-100 transition-all group">
                <div class="flex items-center min-w-max">
                    <svg class="w-5 h-5 flex-shrink-0 transition-colors text-gray-400 group-hover:text-primary-600"
       ...
       class="flex items-center px-3 py-2 text-sm font-medium rounded-xl transition-all duration-200 group
        ...
        <div class="mt-auto pt-4 border-t border-gray-200">
            ...
            <a href="{{ route('meksikoclean.dashboard') }}" 
               class="flex items-center px-3 py-2 text-sm font-medium rounded-xl transition-all duration-200 group
                          {{ request()->routeIs('meksikoclean.*') 
                              ? 'bg-blue-50 text-blue-600' 
                              : 'text-gray-700 hover:bg-gray-100' }}">
                <svg class="w-5 h-5 flex-shrink-0 transition-colors {{ request()->routeIs('meksikoclean.*') ? 'text-blue-600' : 'text-gray-400 group-hover:text-blue-600' }}" 
                     :class="!{{ $isMobile ? 'true' : 'false' }} && sidebarMinimized ? 'mx-auto' : 'mr-3'"
                     fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('meksikoclean.services.index') }}" 
               class="flex items-center px-3 py-2 mt-1 text-sm font-medium rounded-xl transition-all duration-200 group
                          {{ request()->routeIs('meksikoclean.services.*') 
                              ? 'bg-blue-50 text-blue-600' 
                              : 'text-gray-700 hover:bg-gray-100' }}">
                <svg class="w-5 h-5 flex-shrink-0 transition-colors {{ request()->routeIs('meksikoclean.services.*') ? 'text-blue-600' : 'text-gray-400 group-hover:text-blue-600' }}" 
                     :class="!{{ $isMobile ? 'true' : 'false' }} && sidebarMinimized ? 'mx-auto' : 'mr-3'"
                     fill="none" stroke="currentColor" viewBox="0 0 24 24">
                     <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 002-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path>
                </svg>
                <span x-show="{{ $isMobile ? 'true' : '!sidebarMinimized' }}" class="truncate">Daftar Layanan</span>
            </a>

            <a href="{{ route('meksikoclean.partners.index') }}" 
               class="flex items-center px-3 py-2 mt-1 text-sm font-medium rounded-xl transition-all duration-200 group
                          {{ request()->routeIs('meksikoclean.partners.*') 
                              ? 'bg-blue-50 text-blue-600' 
                              : 'text-gray-700 hover:bg-gray-100' }}">
                <svg class="w-5 h-5 flex-shrink-0 transition-colors {{ request()->routeIs('meksikoclean.partners.*') ? 'text-blue-600' : 'text-gray-400 group-hover:text-blue-600' }}" 
                     :class="!{{ $isMobile ? 'true' : 'false' }} && sidebarMinimized ? 'mx-auto' : 'mr-3'"
                     fill="none" stroke="currentColor" viewBox="0 0 24 24">
                     <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path>
                </svg>
                <span x-show="{{ $isMobile ? 'true' : '!sidebarMinimized' }}" class="truncate">Mitra Offline</span>
            </a>

            <a href="{{ route('meksikoclean.transactions.index') }}" 
               class="flex items-center px-3 py-2 mt-1 text-sm font-medium rounded-xl transition-all duration-200 group
                          {{ request()->routeIs('meksikoclean.transactions.*') 
                              ? 'bg-blue-50 text-blue-600' 
                              : 'text-gray-700 hover:bg-gray-100' }}">
                <svg class="w-5 h-5 flex-shrink-0 transition-colors {{ request()->routeIs('meksikoclean.transactions.*') ? 'text-blue-600' : 'text-gray-400 group-hover:text-blue-600' }}" 
                     :class="!{{ $isMobile ? 'true' : 'false' }} && sidebarMinimized ? 'mx-auto' : 'mr-3'"
                     fill="none" stroke="currentColor" viewBox="0 0 24 24">
                     <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                </svg>
                <span x-show="{{ $isMobile ? 'true' : '!sidebarMinimized' }}" class="truncate">Transaksi POS</span>
            </a>
            @endif
        </div>